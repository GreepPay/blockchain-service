...
pub mod account {
    ...
    pub async fn get_account(account_id: &str) -> Result<Account, Error> {
        ...
        let account = models::schema::accounts::table.find(account_uuid).first(&mut db_connection).await?;
    ...
    pub async fn get_many_accounts(page: i64) -> Result<Pagination<Account>, Error> {
        let mut db_connection = establish_connection().await.unwrap();
            let (accounts, total_pages, total_records, per_page) = models::schema::accounts::table
                .into_boxed()
                .paginate(page)
    ...
    pub async fn activate_account(account_id: &str) -> Result<bool, Error> {
        ...
        let (activation_response, funding_account, new_account, amount) = stellar_chain
            .activate_account(account_keypair)
            .await?;

        let amount_to_bigint = amount.to_i64().to_string().parse::<BigDecimal>().unwrap();

        common::save_chain_transaction(activation_response, funding_account, new_account, "XLM".to_string(), amount_to_bigint).await?;
